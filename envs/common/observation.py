...
from highway_env.traffic_light.traffic_light import TrafficLightState, TrafficLightNetwork
...
class TrafficLightObservation(ObservationType):
    ...
    def space(self) -> spaces.Space:
        ...
        return spaces.Dict({
            ...
            # 交差点までの距離
            "distance_to_traffic_light": spaces.Box(
                low=-10, 
                high=100, 
                shape=(1,), 
                dtype=np.float32
...
def observation_factory(env: AbstractEnv, config: dict) -> ObservationType:
    ...
    elif config["type"] == "TrafficLightObservation":
        return TrafficLightObservation(env, **config)
