...
class ControlledVehicle(Vehicle):
    ...
    def plan_route_to(self, destination: str) -> "ControlledVehicle":
        ...
        if path:
            ...
            for i in range(len(path) - 1):
                ...
                # print(f"is in intersection: from {_from_node} to {_to_node}")
    ...
    def follow_road(self) -> None:
        ...
        if self.road.network.get_lane(self.target_lane_index).after_end(self.position):
            ...
            # print(self.target_lane_index, "->", next_lane)
            # if cond:
            #     if hasattr(t_lane, "end"):
            #         print(f"from:{t_lane.end}, ")
            #     else:
            #         print(f"from:{t_lane.position(t_lane.length, 0)},")
            n_lane = self.road.network.get_lane(next_lane)
            # if cond:
            #     print(f"to:{n_lane.start}")
            self.target_lane_index = next_lane
